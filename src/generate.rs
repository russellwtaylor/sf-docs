use crate::{
    aura_parser, cache, cli, custom_metadata_parser, doc_client, flexipage_parser, flow_parser,
    gemini, lwc_parser, object_parser, openai_compat, parser, renderer, scanner, trigger_parser,
    types, validation_rule_parser,
};
...
pub async fn run_generate(args: &cli::GenerateArgs) -> Result<()> {
    ...
    let model: Arc<str> = Arc::from(args.model.as_deref().unwrap_or(provider.default_model()));
    ...
    let custom_metadata_records: Vec<_> = custom_metadata_files
        .par_iter()
        .map(|f| custom_metadata_parser::parse_custom_metadata_record(&f.path, &f.raw_source))
        .collect::<Result<_>>()?;
    ...
    let mut cm_by_type: std::collections::BTreeMap<String, Vec<types::CustomMetadataRecord>> =
    ...
    let object_contexts: Vec<renderer::RenderContext<ObjectMetadata, ObjectDocumentation>> =
        object_files
            .into_iter()
            .zip(object_meta)
            .zip(object_docs)
